/*
 * I don't like that this is in org.jpp.heart, but PySequence has package
...
import org.jpp.astnodes.adapter.AstAdapter;
...
public class AstList extends PySequence implements Cloneable, List, Traverseproc {
    ...
    protected void setsliceList(int start, int stop, int step, List value) {
        if(step != 1) {
            throw Py.TypeError("setslice with java.org.jpp.util.List and step != 1 not supported yet");
        }
        int n = value.size();
        for(int i = 0; i < n; i++) {
