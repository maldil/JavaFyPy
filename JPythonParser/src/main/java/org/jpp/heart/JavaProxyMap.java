...
/**
 * Proxy Java objects implementing java.org.jpp.util.List with Python methods
...
class JavaProxyMap {
    ...
    // "get" needs to override java.org.jpp.util.Map#get() in its subclasses, too, so this needs to be injected last
    ...
    private static final PyBuiltinClassMethodNarrow mapFromKeysProxy = new MapClassMethod("fromkeys", 1, 2) {
        ...
        public PyObject __call__(PyObject keys, PyObject _default) {
            ...
            try {
                // always injected to java.org.jpp.util.Map, so we know the class object we get from asClass is subtype of java.org.jpp.util.Map
