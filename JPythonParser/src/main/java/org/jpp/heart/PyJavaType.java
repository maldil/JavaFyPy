...
public class PyJavaType extends PyType {
    ...
    // java.lang, java.net, java.org.jpp.util that are not marked Cloneable.
    ...
    protected void init(Class<?> forClass, Set<PyJavaType> needsInners) {
        ...
        if (name.startsWith("org.jpp.heart.Py")) {
            name = name.substring("org.jpp.heart.Py".length()).toLowerCase();
        }
        ...
        if (!Modifier.isPublic(forClass.getModifiers()) &&
                !name.startsWith("org/jpp/heart") && Options.respectJavaAccessibility) {
