...
def load_mappings():
    ...
    mappings = {}
    print scriptdir
    for line in open(os.path.join(scriptdir, 'template_files','mappings')):
	if line.strip() is '' or line.startswith('#'):
	    continue
	tmpl, klass = line.strip().split(':')
	print os.path.join(srcdir,'java', *klass.split('.')) + '.java'

	mappings[tmpl] = (os.path.join(scriptdir,'template_files', tmpl),
			  os.path.join(srcdir,'java', *klass.split('.')) + '.java')
    return mappings
