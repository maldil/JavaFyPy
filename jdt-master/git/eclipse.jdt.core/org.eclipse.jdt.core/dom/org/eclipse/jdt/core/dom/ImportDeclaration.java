...
public class ImportDeclaration extends ASTNode {
	...
	public static final ChildPropertyDescriptor AS_NAME_PROPERTY =
			new ChildPropertyDescriptor(ImportDeclaration.class, "name", Name.class, OPTIONAL, NO_CYCLE_RISK);
	/**
	...
	static {
		...
		addProperty(AS_NAME_PROPERTY, properyList);
	...
	private Name importasName=null;
	/**
	...
	public ASTNode internalGetSetChildProperty(ChildPropertyDescriptor property, boolean get, ASTNode child) {
		...
		else if (property == AS_NAME_PROPERTY){
			if (get) {
				return getasName();
			} else {
				setasName((Name) child);
				return null;
			}
		}
		// allow default implementation to flag the error
	...
	public Name getasName()  {
		if (this.importasName == null) {
			// lazy init must be thread-safe for readers
			synchronized (this) {
				if (this.importasName == null) {
					preLazyInit();
					this.importasName =this.ast.newQualifiedName(
							new SimpleName(this.ast), new SimpleName(this.ast));
					postLazyInit(this.importasName, AS_NAME_PROPERTY);
				}
			}
		}
		return this.importasName;
	}
	...
	public void setasName(Name asname) {
		if (asname == null) {
			throw new IllegalArgumentException();
		}
		ASTNode oldChild = this.importasName;
		preReplaceChild(oldChild, asname, AS_NAME_PROPERTY);
		this.importasName = asname;
		postReplaceChild(oldChild, asname, AS_NAME_PROPERTY);
	}

	public void preReplaceChildFromChild(ASTNode oldChild, ASTNode newChild, ChildPropertyDescriptor property){
		preReplaceChild(oldChild,newChild,property);
	}

	public void postReplaceChildFromChild(ASTNode oldChild, ASTNode newChild, ChildPropertyDescriptor property){
		postReplaceChild(oldChild,newChild,property);
	}

	public void preLazyInitFromChild(){
		preLazyInit();
	}

	public void postLazyInitFromChild(ASTNode newChild, ChildPropertyDescriptor property){
		postLazyInit(newChild,property);
	}
	/**
	...
	int treeSize() {
		return
			...
			+ (this.importName == null ? 0 : getName().treeSize()
					+ (this.importasName == null ? 0 : getasName().treeSize()));
	}
}
